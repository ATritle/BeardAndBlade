...
function rock(ctx,x,y,s,angle,a){ctx.save();ctx.translate(Math.round(x),Math.round(y));ctx.rotate(angle);dot(ctx,-s/2,-s/2,s,'#534b42',a);dot(ctx,-s/2,-s/2,s*.7,'#a08a66',a);dot(ctx,s*.15,s*.2,s*.45,'#302d2b',a);ctx.restore();}
