...
Download the Windows game ZIP from this repository's Releases page when available. Extract the entire archive, then launch `TheBeardAndBlade.exe` inside the Windows folder. Keep the Engine and TheBeardAndBlade subfolders alongside the launcher. Unreal Editor is not required. If Windows reports missing Visual C++ runtime components, install the bundled Epic prerequisite installer under `Engine/Extras/Redist/en-us`.
