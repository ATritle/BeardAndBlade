...
Download the Windows game ZIP from this repository's Releases page when available. Extract the entire archive, then launch `BeardAndBlade.exe` inside the Windows folder. Keep the Engine and BeardAndBlade subfolders alongside the launcher. Unreal Editor is not required. If Windows reports missing Visual C++ runtime components, install the bundled Epic prerequisite installer under `Engine/Extras/Redist/en-us`.
...
Clone the repository and open `BeardAndBlade.uproject` in UE 5.8. Install the Visual Studio C++ game-development tools and Windows SDK. All imported runtime art and audio are checked in; the engine, build caches and private reference/source-generation images are not.

Run `Tools/package_windows.ps1` to build, cook, stage and archive a Shipping Windows build. ZIP the entire resulting Windows directory. Do not distribute the editor DLL, the executable in Binaries alone, or GitHub's automatic Source code ZIP as the playable download.
