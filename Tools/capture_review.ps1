...
$argsLine='"'+$projectRoot+'/TheBeardAndBlade.uproject" -game -windowed -ResX=1280 -ResY=800 -DungeonCapture -Dungeon'+$Preview+'Preview -DungeonBiome='+$Biome+' -nosound -unattended -NoSplash -DDC-ForceMemoryCache -LocalDataCachePath="'+$taskRoot+'/work/DDC" -ShaderWorkingDir="'+$taskRoot+'/work/Shaders" -abslog="'+$taskRoot+'/work/render-'+$Preview+$Biome+'.log"'
$argsLine+=' -DDC=InstalledNoZenLocalFallback'
if($Preview -eq 'MenuHover') { $argsLine+=' -DungeonMenuPreview' }
if($Preview -eq 'Controls') { $argsLine+=' -DungeonMenuPreview' }
